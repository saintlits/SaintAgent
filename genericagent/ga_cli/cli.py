...
COMMANDS = {
    ...
    "tui": {
        "help": "启动终端 TUI v2 (Textual)",
        "desc": "启动终端图形界面（多会话、命令面板、历史恢复、流式输出）",
        "cmd": ["python", "{FRONTENDS}/tuiapp_v2.py"],
    },
    "openwarp": {
        "help": "启动 OpenWarp 原生 TUI",
        "desc": "启动为 OpenWarp 终端集成优化的 Textual TUI，可接收 --prompt 初始问题",
        "cmd": ["python", "{FRONTENDS}/tuiapp_v2.py", "--openwarp"],
    },
    "tui-legacy": {
        "help": "启动旧版终端 TUI (tuiapp)",
        "desc": "启动旧版 Textual TUI，仅用于回退排障",
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            示例:
              ...
              ga openwarp          启动 OpenWarp 原生 TUI
              ga openwarp --prompt "修复测试"
              ga pet               启动桌面宠物 v2
