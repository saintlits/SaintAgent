...
⚠ **核心规则：任何 `git reset --hard` 或 `git checkout` 操作前，必须先备份本地文件。**

**正确流程**：
1. **备份** → `git add -A && git commit -m "backup before merge"`（或单独push）
2. **验证 origin 内容** → `git diff --name-only origin/main HEAD`（确认远程有什么）
3. **验证本地内容** → `git diff --name-only HEAD origin/main`（确认本地多什么）
4. **区分文件** → 明确哪些是官方文件（可merge），哪些是用户文件（排除）
5. **执行 merge** → `git reset --hard origin/main` 或 `git merge`
6. **后置验证** → diff 比对，确认所有预期文件都在 origin/main 中
7. **恢复用户文件** → 如有需要，将用户文件重新添加/commit

**禁止行为**：
- ❌ 直接 `git reset --hard origin/main` 不备份（会丢失本地未跟踪文件）
- ❌ 信任 session 总结说"已推送"而不验证（session 622927 的教训）
- ❌ 合并后不验证文件完整性
- ❌ 把用户个人文件（auto_pipeline, hermes 等）推送到公开 repo

**验证命令**：
```bash
# 检查 origin/main 是否包含某个文件
git ls-tree --name-only origin/main memory/ | grep <filename>

# 对比本地和远程
git diff --name-only origin/main HEAD -- memory/
```
