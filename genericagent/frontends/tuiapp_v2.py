...
class GenericAgentTUI(App[None]):
    ...
    def on_mount(self) -> None:
        ...
        welcome = "Welcome to GenericAgent TUI. 按 / 唤起命令面板，Ctrl+N 新建会话。"
        if self.openwarp:
            welcome = "GenericAgent for OpenWarp. 按 / 唤起命令面板，Ctrl+N 新建会话。"
        self._system(welcome)
        ...
        if self.initial_prompt:
            self.call_after_refresh(lambda: self.submit_user_message(self.initial_prompt))
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="GenericAgent TUI v2 (refined visual style)")
    parser.add_argument(
        "--openwarp",
        action="store_true",
        help="启用 OpenWarp 集成入口文案和默认行为",
    )
    parser.add_argument(
        "--prompt",
        default="",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    GenericAgentTUI(initial_prompt=args.prompt, openwarp=args.openwarp).run()
    return 0
