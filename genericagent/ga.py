...
# RTK command mapping: system command -> rtk proxy command
# Only wraps commands that have RTK equivalents with built-in filters
RTK_COMMAND_MAP = {
    'ls': 'rtk ls',
    'tree': 'rtk tree',
    'read': 'rtk read',
...
def _wrap_with_rtk(code, code_type):
    """Wrap bash commands with RTK proxy equivalents if enabled.
    Only applies to simple commands (no pipes, redirects, etc.)"""
    if code_type not in ("bash", "sh", "shell"):
        return code
    rtk_enabled = os.environ.get("RTK_COMMANDS", "0").lower() in ("1", "true", "yes")
    if not rtk_enabled:
        return code
    # Only wrap simple commands (no pipes, redirects, semicolons)
    stripped = code.strip()
    if any(c in stripped for c in ['|', '>', '<', ';', '&', '`', '$(']):
        return code  # Complex command, skip RTK wrapping
    # Extract the command name
    parts = stripped.split()
    if not parts:
        return code
    cmd_name = os.path.basename(parts[0])
    rtk_cmd = RTK_COMMAND_MAP.get(cmd_name)
    if rtk_cmd:
        return rtk_cmd + ' ' + ' '.join(parts[1:])
    return code
...
def code_run(code, code_type="python", timeout=60, cwd=None, code_cwd=None, stop_signal=None, maxlen=10000):
    ...
    code = _wrap_with_rtk(code, code_type)
    ...
    try:
        ...
        # RTK integration: filter output through RTK if enabled
        rtk_filter = os.environ.get("RTK_FILTER", "0").lower() in ("1", "true", "yes")
        rtk_max = int(os.environ.get("RTK_MAX_CHARS", "2000"))
        rtk_ultra = os.environ.get("RTK_ULTRA", "0").lower() in ("1", "true", "yes")
        if rtk_filter:
            try:
                rtk_cmd = ["rtk", "pipe"] + (["--ultra-compact"] if rtk_ultra else [])
                import subprocess as sp
                p = sp.Popen(rtk_cmd, stdin=sp.PIPE, stdout=sp.PIPE, stderr=sp.PIPE)
                filtered, _ = p.communicate(stdout_str.encode("utf-8"), timeout=10)
                if p.returncode == 0 and filtered:
                    stdout_str = filtered.decode("utf-8", errors="replace")
            except Exception:
                pass  # RTK not available or error, use original output
