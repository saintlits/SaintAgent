...
def _openai_stream(sess, messages):
    ...
    if api_mode == "responses":
        ...
        if sess.stream and sess.include_usage: payload["stream_options"] = {"include_usage": True}
...
class BaseSession:
    def __init__(self, cfg):
        ...
        if isinstance(self.stream, str): self.stream = self.stream.lower() in ('true', '1', 'yes')
        ...
        self.include_usage = cfg.get('include_usage', True)
        if isinstance(self.include_usage, str): self.include_usage = self.include_usage.lower() in ('true', '1', 'yes')
