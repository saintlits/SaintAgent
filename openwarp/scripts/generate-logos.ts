 ...
 *
 * 流程:
 *   1. logo.png → assets/logo.svg(以内嵌 PNG 形式保留完整彩色图标)
 *   2. sharp 读源 PNG,按各尺寸光栅化为透明背景 PNG
 *   3. 16/32/48 三个小尺寸合成 icon.ico
 *   4. 写入各 channel 的 no-padding/ 目录,并同步 macOS AppIcon.icon bundle
 *
...
const MAC_ICON_CHANNELS = ["dev", "preview", "stable", "local"] as const;
...
async function writeEmbeddedLogoSvg(sourcePng: Buffer) {
  const meta = await sharp(sourcePng).metadata();
  const width = meta.width ?? 1024;
  const height = meta.height ?? width;
  const href = sourcePng.toString("base64");
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><image width="${width}" height="${height}" href="data:image/png;base64,${href}"/></svg>\n`;
  await fs.writeFile(SVG_OUT, svg, "utf8");
}

async function renderPng(sourcePng: Buffer, size: number): Promise<Buffer> {
...
async function writeMacIconBundle(channel: (typeof MAC_ICON_CHANNELS)[number], sourcePng: Buffer) {
  const iconBundleDir = path.join(REPO_ROOT, "app", "channels", channel, "icon", "AppIcon.icon");
  const assetsDir = path.join(iconBundleDir, "Assets");
  await fs.rm(assetsDir, { recursive: true, force: true });
  await fs.mkdir(assetsDir, { recursive: true });
  await fs.writeFile(path.join(assetsDir, "Logo.png"), sourcePng);
  await fs.writeFile(
    path.join(iconBundleDir, "icon.json"),
    `${JSON.stringify(
      {
        fill: {
...
async function main() {
  ...
  console.log(`[1/4] 读取 ${path.relative(REPO_ROOT, SOURCE_PNG)} 并写入 SVG 包装`);
  const sourcePng = await fs.readFile(SOURCE_PNG);
  await writeEmbeddedLogoSvg(sourcePng);
  ...
  for (const size of PNG_SIZES) {
    pngBySize.set(size, await renderPng(sourcePng, size));
  }
  ...
  for (const ch of MAC_ICON_CHANNELS) {
    await writeMacIconBundle(ch, sourcePng);
    console.log(`      ✓ ${ch}/AppIcon.icon`);
  }
