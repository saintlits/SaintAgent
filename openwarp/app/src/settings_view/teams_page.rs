...
impl std::fmt::Display for TeamsInviteOption {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s: String = match self {
            TeamsInviteOption::Link => crate::t!("settings-teams-tab-link"),
            TeamsInviteOption::Email => crate::t!("settings-teams-tab-email"),
        };
        write!(f, "{s}")
    }
}
...
impl TeamsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<TeamsPageView>) -> Self {
        ...
        let create_team_editor = Self::editor(
            ...
            &crate::t!("settings-teams-team-name-placeholder"),
            font_size,
            ctx,
        );
        ...
        let approve_domains_block_editor = ctx.add_typed_action_view(|ctx| {
            WordBlockEditorView::new(
                ctx,
                &crate::t!("settings-teams-domains-placeholder"),
                font_size,
                vec![',', ' '],
                MAX_CHIP_WIDTH,
        ...
        let email_invites_block_editor = ctx.add_typed_action_view(|ctx| {
            WordBlockEditorView::new(
                ctx,
                &crate::t!("settings-teams-emails-placeholder"),
                font_size,
                vec![',', ' '],
                MAX_CHIP_WIDTH,
        ...
        let rename_team_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(&crate::t!("settings-teams-rename-placeholder"), ctx);
            input
        });
        ...
        let transfer_ownership_modal = ctx.add_typed_action_view(|ctx| {
            Modal::new(
                Some(crate::t!("settings-teams-transfer-modal-title")),
    ...
    fn handle_model_event(
        ...
        match event {
            ...
            UserWorkspacesEvent::ToggleInviteLinksSuccess => {
                self.show_success(crate::t!("settings-teams-toast-toggled-invite-links"), ctx);
                ctx.notify();
            }
            ...
            UserWorkspacesEvent::AddDomainRestrictionsRejected(err) => {
                self.show_error(crate::t!("settings-teams-error-add-domain"), Some(err), ctx)
            }
            ...
            UserWorkspacesEvent::JoinTeamWithTeamDiscoveryRejected(err) => {
                self.show_error(crate::t!("settings-teams-error-join-team"), Some(err), ctx);
            }
            ...
            UserWorkspacesEvent::TransferTeamOwnershipSuccess => {
                self.show_success(crate::t!("settings-teams-toast-transferred-ownership"), ctx);
                ctx.notify();
            }
    ...
    fn add_domain_restrictions(&mut self, team_uid: ServerId, ctx: &mut ViewContext<Self>) {
        ...
        if !invalid_domains.is_empty() {
            let error = crate::t!(
                "settings-teams-toast-invalid-domains",
                count = invalid_domains.len()
            );
        ...
        self.show_success(
            crate::t!(
                "settings-teams-toast-domain-added",
    ...
    fn send_email_invites(&mut self, team_uid: ServerId, ctx: &mut ViewContext<Self>) {
        ...
        if !invalid_emails.is_empty() {
            let error = crate::t!(
                "settings-teams-toast-invalid-emails",
                count = invalid_emails.len()
            );
            self.show_error(error, None, ctx);
            return;
        }
        ...
        let message = if unique_emails.len() == 1 {
            crate::t!("settings-teams-toast-invite-sent-one")
        } else {
            crate::t!(
                "settings-teams-toast-invites-sent",
    ...
    fn team_to_item_list(&self, team: &Team, current_user_email: &str) -> Vec<Item> {
        ...
        team.pending_email_invites.iter().for_each(|email_invite| {
            ...
            let actions = if current_user_has_admin_permissions {
                vec![ItemAction {
                    icon: Icon::X,
                    label: crate::t!("settings-teams-action-cancel-invite"),
            ...
            if member.email != *current_user_email {
                ...
                if current_user_has_owner_permissions && !team_member_has_owner_permissions {
                    actions.push(ItemAction {
                        icon: Icon::Users,
                        label: crate::t!("settings-teams-action-transfer-ownership"),
                    ...
                    && !team_member_has_owner_permissions
                {
                    if team_member_has_admin_permissions {
                        actions.push(ItemAction {
                            icon: Icon::ArrowDown,
                            label: crate::t!("settings-teams-action-demote-from-admin"),
                        ...
                        actions.push(ItemAction {
                            icon: Icon::ArrowUp,
                            label: crate::t!("settings-teams-action-promote-to-admin"),
                            action: TeamsPageAction::SetTeamMemberRole {
                                team_uid: team.uid,
                                user_uid: member.uid,
                ...
                if current_user_has_admin_permissions && !team_member_has_owner_permissions {
                    actions.push(ItemAction {
                        icon: Icon::X,
                        label: crate::t!("settings-teams-action-remove-from-team"),
...
impl TeamsWidget {
        ...
        match team.billing_metadata.delinquency_status {
            DelinquencyStatus::PastDue => {
                left_side.add_child(
                    Container::new(self.render_delinquency_badge(
                        appearance,
                        crate::t!("settings-teams-badge-past-due"),
            ...
            DelinquencyStatus::Unpaid => {
                left_side.add_child(
                    Container::new(self.render_delinquency_badge(
                        appearance,
                        crate::t!("settings-teams-badge-unpaid"),
    ...
    fn render_admin_panel_button(
        ...
        appearance
            ...
            .with_text_and_icon_label(
                TextAndIcon::new(
                    TextAndIconAlignment::IconFirst,
                    crate::t!("settings-teams-open-admin-panel"),
    ...
    fn render_plan_usage(
        ...
        let sub_header_text = match team.billing_metadata.customer_type {
            CustomerType::Free => crate::t!("settings-teams-section-plan-usage-free"),
            _ => crate::t!("settings-teams-section-plan-usage"),
        };
                ...
                section.add_child(
                    Align::new(
                        appearance
                            .ui_builder()
                            .link(
                                crate::t!("settings-teams-reset-links"),
                                None,
        ...
        let domains_as_items: Vec<Item> = team
            ...
            .map(|domain_restriction| {
                let actions = if has_admin_permissions {
                    vec![ItemAction {
                        icon: Icon::X,
                        label: crate::t!("settings-teams-action-remove-domain"),
    ...
    fn render_approve_domains_button(
        ...
        Container::new(self.render_button(
            &crate::t!("settings-teams-set-button"),
            variant,
    ...
    fn render_send_email_invites_button(
        ...
        Container::new(self.render_button(
            &crate::t!("settings-teams-invite-button"),
            variant,
    ...
    fn render_leave_or_delete_team_button(
        ...
        let (label, action) = if is_team_owner {
            (
                crate::t!("settings-teams-delete-team-button"),
                TeamsPageAction::ShowDeleteTeamConfirmationDialog,
            )
        } else {
            (
                crate::t!("settings-teams-leave-team-button"),
                TeamsPageAction::ShowLeaveTeamConfirmationDialog,
            )
        };
                ...
                match item.state {
                    ItemState::Expired => {
                        pending_and_close_row.add_child(
                            self.render_state_chip(
                                appearance,
                                crate::t!("settings-teams-state-expired"),
                    ...
                    ItemState::Pending => {
                        pending_and_close_row.add_child(
                            self.render_state_chip(
                                appearance,
                                crate::t!("settings-teams-state-pending"),
                    ...
                    ItemState::Owner => {
                        pending_and_close_row.add_child(self.render_state_chip(
                            appearance,
                            crate::t!("settings-teams-state-owner"),
                    ...
                    ItemState::Admin => {
                        pending_and_close_row.add_child(
                            self.render_state_chip(
                                appearance,
                                crate::t!("settings-teams-state-admin"),
                                appearance
                                    .theme()
                                    .background()
    ...
    fn render_create_team_page(
        ...
        if !view.discoverable_teams_states.is_empty() {
            ...
            page.add_child(self.render_sub_header_with_subtext_color(
                appearance,
                crate::t!("settings-teams-or-join-existing"),
            ));
    ...
    fn render_single_team_in_team_discovery(
        ...
        let teammate_string = if team_state.team.num_members == 1 {
            crate::t!("settings-teams-discovery-1-teammate")
        } else {
            crate::t!(
                "settings-teams-discovery-n-teammates",
                count = team_state.team.num_members
            )
        };
    ...
    fn render_create_team_button(
        ...
        let mut button = appearance
            ...
            .with_centered_text_label(crate::t!("settings-teams-create-button"))
            .with_style(UiComponentStyles {
                font_color: Some(
                    appearance
                        .theme()
    ...
    fn render_join_team_button(
        ...
        if team_state.team.team_accepting_invites {
            self.render_button(
                &crate::t!("settings-teams-join-button"),
                ButtonVariant::Accent,
            ...
            appearance
                ...
                .with_centered_text_label(crate::t!("settings-teams-contact-admin-button"))
                .disabled()
                .build()
                .finish()
        }
...
impl SettingsWidget for TeamsWidget {
    ...
    fn render(
        ...
        let content = if NetworkStatus::as_ref(app).is_online() {
            ...
            appearance
                .ui_builder()
                .span(crate::t!("settings-teams-offline"))
                .build()
                .finish()
        };
