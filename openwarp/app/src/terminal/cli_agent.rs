...
const GENERIC_AGENT_COLOR: ColorU = ColorU {
    r: 126,
    g: 194,
    b: 126,
    a: 255,
...
/// Represents a CLI agent (e.g., Claude Code, Gemini CLI, Codex, GenericAgent, Amp, Droid, OpenCode, Copilot, Pi, Auggie, Cursor, Goose)
...
impl CLIAgent {
    pub fn generic_agent_openwarp_launch_command(initial_prompt: Option<&str>) -> String {
        let prompt = initial_prompt
            .map(str::trim)
            .filter(|prompt| !prompt.is_empty());

        if cfg!(target_os = "windows") {
            let mut command = "ga openwarp".to_string();
        ...
        let prompt_arg = prompt
            .map(|prompt| format!(" --prompt {}", shell_words::quote(prompt)))
            .unwrap_or_default();

        format!(
            "if command -v ga >/dev/null 2>&1; then ga openwarp{prompt_arg}; else for dir in \"${{GENERIC_AGENT_HOME:-}}\" /Users/Shared/code/app/GenericAgent /Users/saint/code/app/GenericAgent; do if [ -n \"$dir\" ] && [ -f \"$dir/ga_cli/cli.py\" ]; then cd \"$dir\" && python3 -m ga_cli openwarp{prompt_arg}; exit $?; fi; done; python3 -m ga_cli openwarp{prompt_arg}; fi"
        )
    }
    ...
    fn command_prefix_aliases(&self) -> &'static [&'static str] {
        match self {
            ...
            CLIAgent::GenericAgent => &["genericagent", "ga-cli", "ga_cli"],
            _ => &[],
        }
    }
    ...
    pub fn display_name(&self) -> &'static str {
        match self {
            ...
            CLIAgent::Gemini => "Gemini",
            CLIAgent::Codex => "Codex",
            CLIAgent::GenericAgent => "GenericAgent",
            CLIAgent::Amp => "Amp",
            CLIAgent::Droid => "Droid",
            CLIAgent::OpenCode => "OpenCode",
            CLIAgent::Copilot => "Copilot",
    ...
    pub fn icon(&self) -> Option<Icon> {
        match self {
            ...
            CLIAgent::GenericAgent => Some(Icon::Neurology),
    ...
    pub fn supported_skill_providers(&self) -> &'static [SkillProvider] {
        match self {
            ...
            CLIAgent::GenericAgent => &[SkillProvider::Agents],
    ...
    pub fn brand_color(&self) -> Option<ColorU> {
        match self {
            ...
            CLIAgent::GenericAgent => Some(GENERIC_AGENT_COLOR),
        ...
        enum_iterator::all::<CLIAgent>()
            ...
            .find(|agent| {
                agent.matches_command_prefix(&resolved_first_word)
                    ...
                    || (matches!(agent, CLIAgent::GenericAgent)
                        && Self::is_generic_agent_python_command(&resolved_command))
            })
    }

    fn is_generic_agent_python_command(resolved_command: &str) -> bool {
        let command = resolved_command.trim_start();
        command.starts_with("python -m ga_cli")
            || command.starts_with("python3 -m ga_cli")
            || command.contains("/GenericAgent/agentmain.py")
            || command.contains("/GenericAgent/frontends/tuiapp.py")
            || command.contains("/GenericAgent/frontends/tuiapp_v2.py")
    }
...
impl From<CLIAgent> for CLIAgentType {
    fn from(agent: CLIAgent) -> Self {
        match agent {
            ...
            CLIAgent::GenericAgent => CLIAgentType::GenericAgent,
