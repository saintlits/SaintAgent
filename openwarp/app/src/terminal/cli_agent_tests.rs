    ...
    App::test((), |mut app| async move {
        app.update(|ctx| {
            for (command, expected) in [
                ...
                ("ga", CLIAgent::GenericAgent),
                ("genericagent", CLIAgent::GenericAgent),
                ("ga-cli", CLIAgent::GenericAgent),
                ("ga_cli", CLIAgent::GenericAgent),
...
fn test_detect_with_arguments() {
    App::test((), |mut app| async move {
        app.update(|ctx| {
            ...
            assert_eq!(
                CLIAgent::detect("ga tui", None, None, ctx),
                Some(CLIAgent::GenericAgent),
            );
            assert_eq!(
                CLIAgent::detect("ga openwarp", None, None, ctx),
                Some(CLIAgent::GenericAgent),
            );
            assert_eq!(
                CLIAgent::detect("python -m ga_cli tui", None, None, ctx),
                Some(CLIAgent::GenericAgent),
            );
            assert_eq!(
                CLIAgent::detect(
                    "python /Users/saint/code/app/GenericAgent/frontends/tuiapp_v2.py",
                    None,
                    None,
                    ctx,
                ),
...
#[test]
fn generic_agent_openwarp_launch_command_uses_openwarp_entrypoint() {
    let command = CLIAgent::generic_agent_openwarp_launch_command(Some("fix this \"test\""));

    assert!(command.contains("openwarp"));
    assert!(command.contains("--prompt"));
    assert!(command.contains("fix this"));
    assert!(!command.contains("tuiapp_v2.py"));
}
