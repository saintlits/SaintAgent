...
impl CommentEditor {
    ...
    fn create_buttons(
        ...
        let save_button = ctx.add_typed_action_view(|ctx| {
            ActionButton::new(crate::t!("code-comment"), PrimaryTheme)
                .with_keybinding(
        ...
        let remove_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(crate::t!("common-remove"), DangerNakedTheme)
                .on_click(|ctx| {
    ...
    pub fn reopen_saved_comment(
        ...
        self.save_button.update(ctx, |button, ctx| {
            button.set_label(crate::t!("common-update"), ctx);
        });
        ctx.notify();
    ...
    fn reset(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        self.save_button.update(ctx, |button, ctx| {
            button.set_label(crate::t!("code-comment"), ctx);
        });
        ctx.notify();
