...
fn cli_agent_command_prefix_delegates() {
    ...
    assert_eq!(
        SessionType::CliAgent(CLIAgent::GenericAgent).command_prefix(),
        Some("ga")
    );
}
...
#[test]
fn generic_agent_no_worktree_launches_tui() {
    let config = build_tab_config(
        &SessionType::CliAgent(CLIAgent::GenericAgent),
        Path::new("/home/user/project"),
        false,
        true,
    );
    ...
    assert_eq!(commands.len(), 1);
    assert!(commands[0].contains("ga openwarp") || commands[0].contains("ga_cli openwarp"));
}
