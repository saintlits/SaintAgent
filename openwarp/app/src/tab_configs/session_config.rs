    ...
    fn launch_command(&self) -> Option<String> {
        match self {
            ...
            SessionType::CliAgent(CLIAgent::GenericAgent) => {
                Some(CLIAgent::generic_agent_openwarp_launch_command(None))
            }
            SessionType::CliAgent(agent) => Some(agent.command_prefix().to_string()),
        }
    }
...
pub fn build_tab_config(
    ...
    if let Some(command) = session_type.launch_command() {
